...
latest_jpeg = None
latest_frame = None
latest_boxes = []  # Store latest detected boxes
frame_id = 0
last_frame_ts = 0.0
...
DOOR_CLOSE_COOLDOWN_S = 5.0  # Minimum time before door can close after opening
...
MODEL_PATH = os.path.join(REPO_ROOT, "yolov12n-face.pt")
...
# Door close cooldown tracking
door_last_opened = 0.0
door_cooldown_lock = threading.Lock()
...
def detect_with_model(frame):
    ...
    return best_class, best_conf, boxes


def draw_boxes(frame, boxes):
    """Draw bounding boxes and labels on frame"""
    for box_info in boxes:
        x1, y1, x2, y2 = [int(coord) for coord in box_info["box"]]
        ...
        if cls_name == TARGET_CLASS:
            ...
            color = (0, 165, 255)  # Orange for non-target
...
def camera_streamer():
    ...
    while True:
        ...
        with boxes_lock:
            if latest_boxes:
                frame = draw_boxes(frame.copy(), latest_boxes)
...
def detection_worker(client: mqtt.Client):
    global system_status, latest_boxes, door_last_opened
    while True:
        item = trigger_q.get()
        if item is None:
            break
        ...
        for _ in range(FRAMES_TO_CHECK):
            ...
            pred, conf, boxes = detect_with_model(frame)
            if conf > best_conf:
                best_conf = conf
                best_class = pred
                detected_boxes = boxes  # Store the boxes from best detection

            if time.time() - t_start > DETECTION_TIMEOUT_S:
                break

        # Update global boxes for display
        with boxes_lock:
        ...
        if best_class == TARGET_CLASS and best_conf >= CONF_THRESH:
            ...
            # Record door open time for cooldown
            with door_cooldown_lock:
                door_last_opened = time.time()
        else:
            # Check if we're still in cooldown period
            with door_cooldown_lock:
                time_since_open = time.time() - door_last_opened
                in_cooldown = time_since_open < DOOR_CLOSE_COOLDOWN_S

            if in_cooldown:
                # Still in cooldown, don't send close command yet
                remaining = DOOR_CLOSE_COOLDOWN_S - time_since_open
                add_log(f"Close delayed (cooldown: {remaining:.1f}s remaining)")
                status = {"gate": "DENIED", "seq": seq, "reason": "cooldown", "ts": int(time.time())}
                client.publish(TOPIC_STATUS, json.dumps(status), qos=0)
            else:
                # Cooldown expired, can close now
